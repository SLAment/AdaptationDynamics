...
if (rawmax %% 100) {
  maxcov = rawmax - (rawmax %% 100) + 100
} else {
  maxcov = rawmax
}

## Too mamny samples so break by environment
NaCl <- coolviolins(dpf %>% filter(Environment == "NaCl" | Environment == "N"), color = "#4169E1") + ylim(0, maxcov) + geom_hline(yintercept=50, linetype="dashed", color = "black")
EtOH <- coolviolins(dpf %>% filter(Environment == "Ethanol" | Environment == "LE"), color = "#AC1016") + ylim(0, maxcov) + geom_hline(yintercept=50, linetype="dashed", color = "black")
LiAc01 <- coolviolins(dpf %>% filter(Environment == "LiAc0.01" | Environment == "LE"), color = "#800080") + ylim(0, maxcov) + geom_hline(yintercept=50, linetype="dashed", color = "black")
LiAc02 <- coolviolins(dpf %>% filter(Environment == "LiAc0.02" | Environment == "LE"), color = "#228B22") + ylim(0, maxcov) + geom_hline(yintercept=50, linetype="dashed", color = "black")

# Put them back together
alltreats <- plot_grid(NaCl, EtOH, LiAc01, LiAc02, nrow = 2, align="hv")

ggsave(plotcov, plot = alltreats, width = 45, height = 20, units = "cm")
